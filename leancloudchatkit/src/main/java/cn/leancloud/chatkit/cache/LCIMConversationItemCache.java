...
public class LCIMConversationItemCache {
    ...
    public synchronized void initDB(Context context, final String clientId, int nRecentConversations, final AVCallback callback) {
        conversationItemDBHelper = new LCIMLocalStorage(context, clientId, CONVERSATION_ITEM_TABLE_NAME);
        conversationItemMap.clear();
        if (!conversationItemDBHelper.isFromScratch()) {
            Log.d("LeanCloud", "ConversationCache initFromScratch");
            // 首次初始化数据库，从服务器拉去最近的若干对话
            AVIMConversationsQuery query = LCChatKit.getInstance().getClient().getConversationsQuery();
            query.limit(nRecentConversations);
            ...
            query.findInBackground(new AVIMConversationQueryCallback() {
                ...
                public void done(List<AVIMConversation> list, AVIMException e) {
                    ...
                    if (null != list) {
                        ...
                        for (final AVIMConversation conv : list) {
            ...
            Log.d("LeanCloud", "ConversationCache init");
            // 加载数据库中的数据到缓存
            syncData(callback);
        }
    }
            ...
            protected void internalDone0(final List<String> idList, AVException e) {
                conversationItemDBHelper.getData(idList, new AVCallback<List<String>>() {
                    ...
                    protected void internalDone0(final List<String> dataList, AVException e) {
                        ...
                        Log.d("danielluant", "conversationItemMap=" + conversationItemMap.size());
    ...
    public void clearData() {
        conversationItemDBHelper.dropTable();
    }
}
