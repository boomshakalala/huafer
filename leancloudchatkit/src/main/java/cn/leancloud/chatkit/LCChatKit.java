...
public final class LCChatKit {
     ...
     * @param appKey
     */
    public void init(Context context, String appId, String appKey, AVIMTypedMessageHandler<AVIMTypedMessage> msgHandler, LCIMClientEventHandler eventHandler) {
        ...
        // 与网络相关的 handler
        AVIMClient.setClientEventHandler(eventHandler);
    ...
    public void open(final String userId, String tag, final AVIMClientCallback callback) {
        ...
        AVIMClientCallback openCallback = new AVIMClientCallback() {
            ...
            public void done(final AVIMClient avimClient, AVIMException e) {
                if (null == e) {
                    ...
                    LCIMConversationItemCache.getInstance().initDB(AVOSCloud.applicationContext, userId, 25, new AVCallback() {
                        @Override
        ...
        if (AVUtils.isBlankContent(tag)) {
            AVIMClient.getInstance(userId).open(openOption, openCallback);
        } else {
            AVIMClient.getInstance(userId, tag).open(openOption, openCallback);
        }
    }
