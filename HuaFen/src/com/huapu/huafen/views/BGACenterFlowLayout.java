 ...
 * 描述:流式布局， 最后一行居中
 */
public class BGACenterFlowLayout extends ViewGroup {
    ...
    public BGACenterFlowLayout(Context context) {
        this(context, null);
    }

    public BGACenterFlowLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BGACenterFlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            if (mIsDistributionWhiteSpacing && i != rowCount - 1) {
                row.layout(false, false, top);
            } else {
                row.layout(true, false, top);
            }
    ...
    private class Row {
         ...
         * @param top
         */
        public void layout(boolean isCenter, boolean isNeedSplit, int top) {
            if (mViews.size() == 0) {
                return;
            }
            ...
            if(isCenter) {
            	for(int i = 0; i < count; i++) {
            		viewsWidth += mViews.get(i).getWidth();
            	}
//            	left = (CommonUtils.getScreenWidth() / 2) - viewsWidth / 2 - mHorizontalChildGap;
            	left = (CommonUtils.getScreenWidth() - viewsWidth) / 2 - mHorizontalChildGap;
            }
            View view;
            for (int i = 0; i < count; i++) {
                view = mViews.get(i);
