 ...
 * needed.
 * 
 * It also supports pagination by setting a custom view as the loading indicator.
 */
...
public class AmazingListView extends ListView {
    ...
    public void configureHeaderView(int position) { //onScroll里调用
        if (mHeaderView == null || adapter == null) {
            return;
        }
		position -= getHeaderViewsCount();//加入header position的判断
...
//					QLog.i(QLog.FF,0+", "+y+", "+mHeaderViewWidth+", "+(mHeaderViewHeight + y));
				}
                mHeaderViewVisible = true;
            }
            break;
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (!(adapter instanceof AmazingAdapter)) {
            throw new IllegalArgumentException(AmazingListView.class.getSimpleName() + " must use adapter of type "
