...
public class ClassificationListAdapter extends CommonWrapper<ClassificationListAdapter.SearchResultListHolder> {
    ...
    public ClassificationListAdapter(Context mContext) {
        super();
    ...
    public SearchResultListHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new SearchResultListHolder(LayoutInflater.from(mContext).inflate(R.layout.classification_list, parent, false));
    }
    ...
    public void onBindViewHolder(final SearchResultListHolder holder, final int position) {
        ...
        if (userData != null && !TextUtils.isEmpty(userData.getAvatarUrl())) {
            holder.ivHeader.setImageURI(userData.getAvatarUrl());
        ...
        if (goodsData != null) {
            ...
            if (!TextUtils.isEmpty(cover)) {
                ...
                ImageLoader.resizeMiddle(holder.ivProPic, cover, 1);
            } else {
                if (!ArrayUtil.isEmpty(goodsData.getGoodsImgs())) {
                    ImageLoader.resizeMiddle(holder.ivProPic, goodsData.getGoodsImgs().get(0), 1);
                }
        ...
        boolean flag = position == data.size() - 1;
        if (flag) {
            holder.classLine.setVisibility(View.INVISIBLE);
        } else {
            holder.classLine.setVisibility(View.VISIBLE);
        }
